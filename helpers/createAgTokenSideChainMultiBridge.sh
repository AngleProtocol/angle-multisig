...
function main {
    ...
    echo ""
    echo "Would you like to create the multisig transaction for the agToken side chain multi bridge ? (yes/no)"

    read createTransaction

    if [[ $createTransaction == "yes" ]]; then

        forge script ConnectAgTokenSideChainMultiBridge --fork-url $mainnet_uri
        if [ $? -ne 0 ]; then
            echo ""
        ...
        forge test --match-contract ConnectAgTokenTokenSideChainMultiBridgesTest
        if [ $? -ne 0 ]; then
            echo ""
        ...
        echo ""
        echo "Would you like to create the transaction ? (yes/no)"
        read execute
