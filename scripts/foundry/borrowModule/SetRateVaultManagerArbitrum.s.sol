...
contract SetRateVaultManagerArbitrum is Utils {
    function run() external {
        ...
        vm.startBroadcast(address(guardianArbitrumSafe));
        address(multiSendArbitrum).delegatecall(payloadMultiSend);
        vm.stopBroadcast();

        _serializeJson(CHAIN_ARBITRUM, address(multiSendArbitrum), 0, payloadMultiSend, Enum.Operation.DelegateCall);
    }
}
