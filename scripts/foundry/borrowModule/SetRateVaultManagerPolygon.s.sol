...
contract SetRateVaultManagerPolygon is Utils {
    function run() external {
        ...
        while (true) {
            try treasuryPolygon.vaultManagerList(i) returns (address vault) {
                uint64 rate;
                // Non yield bearing vaults
                if (i==0 || i==1 || i==2 || i==3 || i==4) rate = twoPoint5Rate;
                else rate = fourRate;
        ...
        address(multiSendPolygon).delegatecall(payloadMultiSend);
        vm.stopBroadcast();
