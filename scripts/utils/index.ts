...
export async function generic(toAddress: string, contractInterface, functionName: string, parameters) {
...
export const gnosisGetSignatures = async (
  ...
  const safeAPI = getSafeAPI(chainId);
  try {
    const resp = await axios.get(`${safeAPI}/multisig-transactions/${safe_tx_hash}/confirmations/`);
    console.log('');
    console.log(resp.data);
    ...
    if (resp.data.count == numConfirmations) {
      executable = true;
      order = parseInt(resp.data.results[0], 16) <= parseInt(resp.data.results[1], 16);
